...
def embed_from_feat(feat_seq, y=None):
    ...
    skew_ = skew(feat_seq, axis=0)
    kurt_ = kurtosis(feat_seq, axis=0)

    # 检查是否有NaN或无穷大值
    if np.isnan(mean_).any() or np.isnan(std_).any() or np.isnan(skew_).any() or np.isnan(kurt_).any():
        print("[ERROR] 统计特征中包含NaN值")
        return None

    if np.isinf(mean_).any() or np.isinf(std_).any() or np.isinf(skew_).any() or np.isinf(kurt_).any():
        print("[ERROR] 统计特征中包含无穷大值")
        return None
        ...
        if len(embs) > 1:
            ...
            for i in range(len(embs)):
                for j in range(i + 1, len(embs)):
                    cos_sim = np.dot(embs[i], embs[j]) / (np.linalg.norm(embs[i]) * np.linalg.norm(embs[j]) + EPS)
